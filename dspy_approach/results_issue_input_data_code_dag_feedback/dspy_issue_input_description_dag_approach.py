...
dags_code = []
for dag in dags:
    code = read_code_from_file(dag)
    dags_code.append(code)

for i, issue in enumerate(issue_names):
    data[i]["issue_name"] = issue
    data[i]["dag"] = dags_code[i]
...
def classify_with_single_model(df, model_name_index):
    if model_name_index == 0:
        ...
        lm = dspy.LM('openai/deepseek/deepseek-r1', api_key=together_api_key,
                     api_base="https://api.together.xyz/v1", cache=False)
...
# dave the relevant codes and fix for the true detected issues
issue_fixes = []
models = ["deepseek_v3", "deepseek_r1_reasoner", "gpt-4o-mini", "gemini-2.0-flash-001", "gpt-4o"]

def save_relevant_code_and_fix():
    for model in models:
...
#save to csv file
save_relevant_code_and_fix()
df_issue_fixes = pd.DataFrame(issue_fixes)
df_issue_fixes.to_csv("issue_fixes.csv", index=False)
